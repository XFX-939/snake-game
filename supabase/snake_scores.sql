...
create index if not exists snake_scores_top10_idx
...
drop policy if exists "Anyone can insert valid snake scores" on public.snake_scores;
create policy "Anyone can insert valid snake scores"
  on public.snake_scores
  for insert
  to anon
  with check (
    player_name ~ '^[A-Za-z0-9_一-龥]{2,12}$'
    and score >= 0
    and score <= 99999
